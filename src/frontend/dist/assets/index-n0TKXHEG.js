...
const Coins = createLucideIcon("coins", __iconNode$4);
/**
 * @license lucide-react v0.511.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const Plane = createLucideIcon("plane", __iconNode$3);
/**
 * @license lucide-react v0.511.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
function generateCrashPoint() {
  ...
  if (r2 < 0.3) return 1 + Math.random() * 0.9;
  const raw = Math.max(1, 0.99 / Math.random());
  return Math.min(raw, 100);
}
function multiplierAtTick(tick) {
  return 1 + (tick * 0.028) ** 1.5;
}
function planePosition(progress) {
  const t = Math.min(progress, 1);
  const p0 = { x: 8, y: 88 };
  const p1 = { x: 40, y: 55 };
  const p2 = { x: 90, y: 12 };
  const x2 = (1 - t) * (1 - t) * p0.x + 2 * (1 - t) * t * p1.x + t * t * p2.x;
  const y = (1 - t) * (1 - t) * p0.y + 2 * (1 - t) * t * p1.y + t * t * p2.y;
  const dt = 0.02;
  const t2 = Math.min(t + dt, 1);
  const x22 = (1 - t2) * (1 - t2) * p0.x + 2 * (1 - t2) * t2 * p1.x + t2 * t2 * p2.x;
  const y2 = (1 - t2) * (1 - t2) * p0.y + 2 * (1 - t2) * t2 * p1.y + t2 * t2 * p2.y;
  const angle = Math.atan2(-(y2 - y), x22 - x2) * (180 / Math.PI);
  return { x: x2, y, angle };
}
function drawFlightPath(ctx, width, height, progress, crashed) {
  ...
  const bezierX = (t) => (1 - t) * (1 - t) * p0.x + 2 * (1 - t) * t * p1.x + t * t * p2.x;
  const bezierY = (t) => (1 - t) * (1 - t) * p0.y + 2 * (1 - t) * t * p1.y + t * t * p2.y;
  ctx.beginPath();
  ctx.moveTo(p0.x, p0.y);
  for (let i = 1; i <= steps; i++) {
    ctx.lineTo(bezierX(i / steps * tMax), bezierY(i / steps * tMax));
  }
  ctx.strokeStyle = crashed ? "oklch(55 0.25 25 / 0.25)" : "oklch(82 0.22 195 / 0.2)";
  ctx.lineWidth = 10;
  ctx.lineCap = "round";
  ctx.stroke();
  ctx.beginPath();
  ...
  ctx.strokeStyle = crashed ? "oklch(55 0.25 25 / 0.8)" : "oklch(82 0.22 195 / 0.8)";
  ctx.lineWidth = 2.5;
  ctx.stroke();
...
function AviatorGame() {
  ...
  const [phase, setPhase] = reactExports.useState("waiting");
  const [countdown, setCountdown] = reactExports.useState(COUNTDOWN_SECS);
  const [multiplier, setMultiplier] = reactExports.useState(1);
  const [crashPoint, setCrashPoint] = reactExports.useState(null);
  const [betInput, setBetInput] = reactExports.useState("50");
  const [autoInput, setAutoInput] = reactExports.useState("");
  const [activeBet, setActiveBet] = reactExports.useState(null);
  const [cashedOutAt, setCashedOutAt] = reactExports.useState(null);
  const [history, setHistory] = reactExports.useState([]);
  const [roundLog, setRoundLog] = reactExports.useState([]);
  const [flashClass, setFlashClass] = reactExports.useState("");
  const [winPopup, setWinPopup] = reactExports.useState(null);
  const [isPlaneExploding, setIsPlaneExploding] = reactExports.useState(false);
  const [leaderboardOpen, setLeaderboardOpen] = reactExports.useState(false);
  const [showHistory, setShowHistory] = reactExports.useState(false);
  const canvasRef = reactExports.useRef(null);
  const intervalRef = reactExports.useRef(null);
  const tickRef = reactExports.useRef(0);
  const crashPointRef = reactExports.useRef(null);
  const activeBetRef = reactExports.useRef(null);
  const cashedOutRef = reactExports.useRef(false);
  const autoInputRef = reactExports.useRef("");
  reactExports.useEffect(() => {
  ...
  reactExports.useEffect(() => {
    ...
    const progress = crashPoint ? Math.min((multiplier - 1) / Math.max(crashPoint - 1, 0.01), 1) : (multiplier - 1) / 10;
    drawFlightPath(ctx, canvas.width, canvas.height, progress, crashed);
  }, [multiplier, phase, crashPoint]);
  const startCountdown = reactExports.useCallback(() => {
    setPhase("waiting");
  ...
  const handleCashout = reactExports.useCallback((currentM) => {
    ...
    ue.success(`Cashed out at ${m.toFixed(2)}x — Won ${winnings} coins!`);
    setTimeout(() => setWinPopup(null), 1600);
  }, []);
  const startFlying = reactExports.useCallback(() => {
  ...
  const placeBet = reactExports.useCallback(() => {
    ...
    setActiveBet(bet);
    cashedOutRef.current = false;
    ue.success(`Bet placed: ${bet} coins`);
  }, [betInput, balance]);
  reactExports.useEffect(() => {
  ...
  const resetBalance = () => {
    ...
    ue.success("Balance reset to 1000 coins");
  };
  const flyProgress = phase === "waiting" ? 0 : crashPoint ? Math.min((multiplier - 1) / Math.max(crashPoint - 1, 0.01), 1) : 0;
  const planePos = planePosition(flyProgress);
  const multiplierColor = phase === "crashed" ? "text-destructive" : cashedOutAt ? "text-success" : "text-primary";
  const multiplierClass = `multiplier-display ${phase === "crashed" ? "crashed" : cashedOutAt ? "won" : ""}`;
  const currentWin = activeBet !== null && phase === "flying" && !cashedOutAt ? Number.parseFloat((activeBet * multiplier).toFixed(2)) : null;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "relative z-10 flex items-center justify-between px-4 py-3 border-b border-border/40 bg-card/60 backdrop-blur-md", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-8 h-8 rounded-lg bg-primary/20 border border-primary/40 flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Plane, { className: "w-4 h-4 text-primary" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "font-display text-lg font-bold text-foreground neon-text tracking-tight", children: "AVIATOR" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                history.slice(0, 10).map((h, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "span",
                  {
                    className: `text-xs font-bold px-2 py-0.5 rounded-full shrink-0 ${h >= 2 ? "history-badge-high" : h >= 1.5 ? "history-badge-mid" : "history-badge-low"}`,
                    children: [
                      h.toFixed(2),
                      "x"
                    ]
                ...
                history.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-muted-foreground", children: "No rounds yet" })
              ]
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Coins, { className: "w-4 h-4 text-primary" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "span",
              {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "relative flex-1 flex flex-col", style: { minHeight: 0 }, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative flex-1", style: { minHeight: "280px" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "canvas",
              {
                ref: canvasRef,
            ...
            phase !== "waiting" && /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
                className: `absolute pointer-events-none ${isPlaneExploding ? "plane-explode" : ""}`,
                style: {
                  left: `${planePos.x}%`,
                  top: `${planePos.y}%`,
                  transform: `translate(-50%, -50%) rotate(${-planePos.angle}deg)`,
                  fontSize: "2.2rem",
                  filter: isPlaneExploding ? "drop-shadow(0 0 16px oklch(55 0.25 25))" : "drop-shadow(0 0 12px oklch(82 0.22 195 / 0.9))"
                },
                children: isPlaneExploding ? "💥" : "✈️"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute inset-0 flex flex-col items-center justify-center pointer-events-none", children: [
              phase === "waiting" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-sm mb-2 tracking-widest uppercase font-bold", children: "Next round in" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "div",
                  {
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-xs mt-2", children: "Place your bets now" })
              ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
                phase === "crashed" && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-destructive text-sm font-bold mb-1 tracking-widest uppercase", children: "FLEW AWAY!" }),
                cashedOutAt && phase === "flying" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "p",
                  {
                ...
                cashedOutAt && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "p",
                  {
              ...
              winPopup && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-8 text-xs text-muted-foreground/40 pointer-events-none", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "1.00x" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "2.00x" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "5.00x" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "10.00x" })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative z-10 p-3 md:p-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-3", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bet-panel rounded-xl p-4 md:col-span-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3 mb-3", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(Label, { className: "text-xs text-muted-foreground mb-1.5 block", children: "Bet Amount" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative", children: [
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground", children: "coins" })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-1.5 mt-1.5", children: QUICK_BETS.map((v2) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "button",
                      {
                        type: "button",
                        onClick: () => setBetInput(v2.toString()),
                        disabled: phase !== "waiting" || activeBet !== null,
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(Label, { className: "text-xs text-muted-foreground mb-1.5 block", children: "Auto Cashout (optional)" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        Input,
                        {
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground", children: "x" })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground/60 mt-1.5", children: "Auto cashout at this multiplier" })
                  ] })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
                  phase === "waiting" && activeBet === null && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    Button,
                    {
                      onClick: placeBet,
                  ...
                  phase === "waiting" && activeBet !== null && /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "div",
                    {
                  ...
                  phase === "flying" && activeBet !== null && cashedOutAt === null && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    Button,
                    {
                  ...
                  phase === "flying" && (activeBet === null || cashedOutAt !== null) && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex items-center justify-center h-12 rounded-lg bg-muted/40 border border-border/40", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-muted-foreground text-sm", children: cashedOutAt ? `Cashed at ${cashedOutAt.toFixed(2)}x ✓` : "No bet this round" }) }),
                  phase === "crashed" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex items-center justify-center h-12 rounded-lg bg-destructive/10 border border-destructive/30", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-destructive font-bold", children: [
                    "Crashed @ ",
                    crashPoint == null ? void 0 : crashPoint.toFixed(2),
                    "x"
                  ] }) })
                ] })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bet-panel rounded-xl p-4 flex flex-col justify-between", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-display text-xs uppercase tracking-widest text-muted-foreground", children: "Stats" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-muted-foreground", children: "Balance" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-bold text-primary font-display", children: balance.toLocaleString() })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-muted-foreground", children: "Current Bet" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-bold text-foreground", children: activeBet ?? "—" })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-muted-foreground", children: "Multiplier" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "span",
                        {
                          className: `font-bold font-display ${multiplierColor}`,
                          children: phase === "waiting" ? "—" : `${multiplier.toFixed(2)}x`
                        }
                      )
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-muted-foreground", children: "Potential Win" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "span",
                        {
                          className: "font-bold",
                          style: { color: "oklch(80 0.22 145)" },
                          children: currentWin !== null ? `${currentWin}` : "—"
                        }
                      )
                    ] })
                  ] })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-3 pt-3 border-t border-border/30", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between text-xs text-muted-foreground", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Rounds played" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: history.length })
                ] }) })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-w-4xl mx-auto mt-3", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
              Collapsible,
              {
                ...
                children: [
                    ...
                    "button",
                    {
                      ...
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
                          ...
                          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Leaderboard" })
                        ] }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          ChevronDown,
                          {
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx(CollapsibleContent, { children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-1 bet-panel rounded-xl overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 divide-x divide-y divide-border/30", children: MOCK_LEADERS.map((p, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "div",
                    {
                      className: "flex items-center gap-3 p-3",
                      "data-ocid": `leaderboard.item.${i + 1}`,
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-2xl", children: p.avatar }),
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5", children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsxs(
                              "span",
                              {
                                className: `text-xs font-bold ${i === 0 ? "text-yellow-400" : i === 1 ? "text-slate-400" : i === 2 ? "text-amber-600" : "text-muted-foreground"}`,
                                children: [
                                  "#",
                                  i + 1
                                ]
                          ...
                          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-primary font-bold", children: p.balance.toLocaleString() })
                        ] })
                      ]
                    },
                    p.name
            ...
            roundLog.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-w-4xl mx-auto mt-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Collapsible, { open: showHistory, onOpenChange: setShowHistory, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(CollapsibleTrigger, { asChild: true, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  type: "button",
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Round History" })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      ChevronDown,
                      {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(CollapsibleContent, { children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-1 bet-panel rounded-xl p-3", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ScrollArea, { className: "h-32", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-1.5", children: roundLog.map((r2, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "span",
                {
                  className: `text-xs font-bold px-2.5 py-1 rounded-full ${r2.crashPoint >= 2 ? "history-badge-high" : r2.crashPoint >= 1.5 ? "history-badge-mid" : "history-badge-low"}`,
                  "data-ocid": `history.item.${i + 1}`,
                  children: [
                    r2.crashPoint.toFixed(2),
                    "x"
                  ]
                },
                i
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("footer", { className: "relative z-10 py-3 text-center text-xs text-muted-foreground/50 border-t border-border/20", children: [
          "© ",
...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(AviatorGame, {}),
